...
More likely that you're wrong, simply from the view of probability.
...
```{r data_manhattan}
manhattan <- read_csv("~/R/lecture-note-class-13-abhikolli/manhattan.csv")
```
...
What does this mean?
This means that 95% of the values are between 2299 and 2977.
...
manhattan %>%
  specify(response = rent) %>%
  generate(repts = 10000, type = "bootstrap") %>%
...
- Population: Mice that died while on treatment
...
What does this mean? 
95% of the values are between .690 and .793.
